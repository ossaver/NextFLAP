...
float SASAction::evaluateMinDuration(SASNumericExpression* e)
{
	...
	if (e->type == 'C') return controlVars[e->var].minValue;
...
float SASAction::evaluateMaxDuration(SASNumericExpression* e)
{
	...
	if (e->type == 'C') return controlVars[e->var].maxValue;
...
void SASAction::postProcess()
{
	...
	for (SASControlVar& cv: controlVars) {
		calc.cvarValues.emplace_back(cv.minValue, cv.maxValue);
	}
	calc.cvarEvaluated.resize(controlVars.size());
}
