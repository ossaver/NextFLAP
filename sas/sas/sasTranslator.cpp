...
void SASTranslator::computeMutex(GroundedAction* a, const vector<unsigned int> preconditions, unsigned int startEndPrec) {
#ifdef	DEBUG_SASTRANS_ON
...
void SASTranslator::generateControlVar(SASAction* a, GroundedControlVar* cv)
{
	...
	scv.minValue = -FLOAT_INFINITY;
	scv.maxValue = FLOAT_INFINITY;
...
char SASTranslator::generateNumericExpressionType(int type) {
	...
	default:
		cout << "Invalid numeric expression type" << endl;
		assert(false);
		c = '\0';
	}
	return c;
...
SASGoalDescription SASTranslator::generateGoalDescription(GroundedGoalDescription* gd, SASTask* sTask, LiteralTranslation* trans) {
	...
	case GG_COMP:
		...
		break;
	default:
		cout << "Unexpected goal description type: " << gd->type << endl;
		//cout << gd->toString(gTask->task, gTask->variables) << endl;
		assert(false);
		g.type = '\0';
	}
	return g;
...
SASConstraint SASTranslator::createConstraint(GroundedConstraint* gc, SASTask* sTask, LiteralTranslation* trans) {
	...
	case RT_GOAL_PREFERENCE:
		...
		break;
	default:
		cout << "Unexpected constraint type " << gc->type << endl;
		assert(false);
		c.type = '\0';
	}
	return c;
