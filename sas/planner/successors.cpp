	...
	TTimePoint startTimeNewAction)
{
	if (supportedConditions(a)) {	 // Check if the (non-numeric) action preconditions can be supported by the steps in the current base plan
		...
		if (numSupportState != -2) { // Supported
			...
			PlanBuilder pb(a, newStep, &matrix, numSupportState, &planEffects);
			unsigned int n = 0;
			if (var != MAX_UINT16) {
...
void Successors::addNumericSupport(PlanBuilder* pb, int numCond, std::vector<TTimePoint>* supportingTimePoints)
{
	...
	if (numCond < atStart.size()) {
		...
		if (pb->addNumLink(v, tp, stepToStartPoint(newStep), task)) {
		...
		if (n < atEnd.size()) {
			TTimePoint tp = supportingTimePoints->at(numCond);
			TVariable v = atEnd[n];
			if (pb->addNumLink(v, tp, stepToEndPoint(newStep), task)) {
				addNumericSupport(pb, numCond + 1, supportingTimePoints);
				pb->removeLastLink();
			}
		}
...
bool Successors::supportedNumericConditions(SASAction* a, int numState) {
	std::vector<SASControlVar>* cvs = &(a->controlVars);
	for (unsigned int i = 0; i < a->startNumCond.size(); i++)
		if (!supportedNumericCondition(a->startNumCond[i], numState, cvs))
			return false;
	for (unsigned int i = 0; i < a->overNumCond.size(); i++)
		if (!supportedNumericCondition(a->overNumCond[i], numState, cvs))
			return false;
	//if (forceAtEndConditions) {
		for (unsigned int i = 0; i < a->endNumCond.size(); i++)
			if (!supportedNumericCondition(a->endNumCond[i], numState, cvs))
				return false;
	//}
	return true;
}
...
int Successors::supportedNumericConditions(SASAction* a)
{
	...
	for (int i = (int)planEffects.numStates.size() - 1; i >= 0; i--) {
		if (supportedNumericConditions(a, i)) 
			return i;
	}
...
// Does not take control vars. of the current action into account
bool Successors::supportedNumericCondition(const SASNumericCondition& c, int stateIndex, std::vector<SASControlVar>* cvs)
{
	if (c.terms[0].type == 'C')
		return true;
	...
	case '=': // [2, 4] = [3, 5] -> [max(2, 3), min(4, 5)] = [3, 4] -> Ok si intervalo no vacío
		min1 = planEffects.getMinValue(&(c.terms.at(0)), stateIndex, cvs);
		if (min1 == FLOAT_UNKNOWN) return false;
		max1 = planEffects.getMaxValue(&(c.terms.at(0)), stateIndex, cvs);
		if (max1 == FLOAT_UNKNOWN) return false;
		min2 = planEffects.getMinValue(&(c.terms.at(1)), stateIndex, cvs);
		if (min2 == FLOAT_UNKNOWN) return false;
		max2 = planEffects.getMaxValue(&(c.terms.at(1)), stateIndex, cvs);
		if (max2 == FLOAT_UNKNOWN) return false;
