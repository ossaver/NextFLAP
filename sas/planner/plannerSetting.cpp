...
Plan* PlannerSetting::plan(float bestMakespan) {
	if (planner == nullptr) {
...
std::string PlannerSetting::planToPDDL(Plan* p, double* makespan, int* numSteps) {
	return planner->planToPDDL(p, makespan, numSteps);
}
