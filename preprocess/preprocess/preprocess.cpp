...
void Preprocess::removeConditionalEffects(Action a, vector<Action> &aList) {
...
// Removes the first conditional effect from the action effect. Returns true when the conditional effect has been deleted
...
// Removes the first conditional effect from the action effect. Returns true when the conditional effect has been deleted
bool Preprocess::removeConditionalEffect(DurativeEffect *eff, DurativeEffect *parent, int numTerm, DurativeAction &a) {
    bool removed = false;
    switch (eff->type) {
    case DET_AND:
        for (unsigned int i = 0; i < eff->terms.size() && !removed; i++)
...
// For the first conditional effect CE=(when P Q), includes P in the preconditions and replaces CE by Q in the effects
    ...
    case ET_WHEN:
        {
            ...
            if (parent != nullptr) parent->terms[numTerm] = q;
            else a.effect = q;
            managed = true;
        }
        break;
    ...
    case ET_NEG_LITERAL:
        {
            ...
            op.addLiteralToEffects(effect.literal, value, AT_END);
        }
        break;
    case ET_AND:
