...
public:
	...
	void updateMax(float v) { if (v < maxValue) maxValue = v; }
	void updateMin(float v) { if (v > minValue) minValue = v; }
};
...
class SASActionCalculations {
public:
	bool durationEvaluated;
	float minDuration;
	float maxDuration;
	std::vector<TInterval> cvarValues;
	std::vector<bool> cvarEvaluated;
	void reset() {
		durationEvaluated = false;
		for (int i = 0; i < cvarEvaluated.size(); i++) cvarEvaluated[i] = false;
	}
};
...
public:
    ...
    std::vector<SASNumericEffect> endNumEff;
  	...
  	bool isGoal;
  	bool isTIL;
	SASActionCalculations calc; // For temporal calculations
	...
	void setGoalCost() {					// Sets the cost and duration of a fictitious goal action
		fixedDuration = true;
