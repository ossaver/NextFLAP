...
void NumericRPG::programActionEffects(SASAction* a, int level)
{
	...
	if (!ic.supportedNumericStartConditions()) return;
	//ic.constrainAtStartFluents();
	ic.applyStartEffects(&svc);
	ic.applyEndEffects(&evc);
	if (!ic.supportedNumericEndConditions()) return;
